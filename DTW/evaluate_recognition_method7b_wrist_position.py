"""
Evaluate Method 7b: M7 (angles + relative lengths) + raw wrist position (2D per hand).
"""
...
CACHE_DIR   = BASE_DIR / "cache" / "recognition_method7b"

sys.path.append(str(BASE_DIR))
from method7b_wrist_position import (  # noqa: E402
...
def _feat_cache_path(video_path: Path, thr: float) -> Path:
    key = video_path.parent.name.replace(" ", "_") + "_" + video_path.name.replace(" ", "_")
    return CACHE_DIR / f"{key}_thr{thr:.3f}.feat.npz"


def extract_features_with_cache(
    video_path: Path, thr: float
) -> Tuple[np.ndarray, List[int]]:
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache_path = _feat_cache_path(video_path, thr)

    if cache_path.exists():
        data = np.load(cache_path, allow_pickle=True)
        return data["features"].astype(np.float32, copy=False), data["indices"].tolist()

    feats, kept_idx = extract_video_features(str(video_path), similarity_threshold=thr)
    if feats.size > 0:
        np.savez_compressed(
            cache_path,
            features=feats,
...
def compare_user_vs_all(
    ...
    for folder, (t_feats, t_idx) in translator_data.items():
        if t_feats.size == 0:
            continue
        r = dtw_global_align(
            t_feats, user_feats, t_idx, user_idx,
            window_ratio=window_ratio, swap_mode=swap,
        )
...
def run_eval(
    ...
    top1_correct = top3_correct = top5_correct = total_users = 0
    ...
    for f_idx, (folder, info) in enumerate(folder_list, 1):
        ...
        for user_path in user_videos:
            ...
            print("  Method: M7b (angles + rel_len + wrist_xy, L1 DTW)")
            ...
            result = {
                "method": "method7b_wrist_position",
                "folder": folder,
...
def save_results(all_results: List[Dict], summary: Dict) -> None:
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for r in all_results:
            writer.writerow({k: v for k, v in r.items() if k not in ("top5_rankings", "method")})
    ...
    with txt_path.open("w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("SIGN LANGUAGE RECOGNITION — Method 7b (angles + rel_len + wrist_xy, L1 DTW)\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"Feature dim: 424 (212 per hand: 190 angles + 20 rel_len + 2 wrist_xy)\n")
        f.write(f"Wrist encoding: image-normalised (x,y) centred at 0.5 → range [-0.5, 0.5]\n")
        ...
        f.write(f"  Method 7b (+ wrist position):           Top-1={summary['top1_accuracy']}%  Top-3={summary['top3_accuracy']}%  Top-5={summary['top5_accuracy']}%\n")
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Evaluate Method 7b (M7 + raw wrist position, L1 DTW)."
    )
    ...
    print("=" * 80)
    print("PHASE 1: Precomputing translator features (M7b, cached)")
    print("=" * 80)
