"""
Evaluate recognition accuracy using Method 5:
  DTW-aligned filtered-frame cosine similarity, with global user hand-swap invariance.

This keeps Method 1–3 untouched and adds an experimental method for research.
"""
...
from method5_dtw_cos_swap import dtw_best_of_user_swap  # noqa: E402


VIDEOS_DIR = os.path.join(os.path.dirname(__file__), "Videos")
CACHE_DIR = os.path.join(os.path.dirname(__file__), "cache", "recognition_method5_dtw_swap")
...
def compare_user_vs_all_translators_m5(
    ...
    *,
    ...
    """
    Returns list of (folder, score, variant) sorted by score desc.
    variant is "orig" or "swap" for the chosen best-of-two DTW run.
    """
    ...
    for folder, (trans_feat, trans_idx) in translator_data.items():
        ...
        results.append((folder, float(r.score), r.variant))
    ...
    *,
    ...
    for f_idx, (folder, info) in enumerate(folder_list, 1):
        ...
        for user_path in user_videos:
            ...
            print("  Method: M5 (DTW+Cos with global user hand-swap)")
            ...
            t1 = time.time()
            rankings = compare_user_vs_all_translators_m5(
                user_feat,
                user_idx,
                translator_data,
                window_ratio=window_ratio,
                verbose_dtw=verbose_dtw,
            )
            ...
            correct_score = rankings[rank - 1][1] if rank <= len(rankings) else 0.0
            correct_variant = rankings[rank - 1][2] if rank <= len(rankings) else "NA"
            ...
            result = {
                ...
                "top1_variant": rankings[0][2],
                "top5_rankings": [(r[0], round(float(r[1]), 4), r[2]) for r in rankings[:5]],
            ...
            print(f"\n  Result: {status}")
            print(f"  Correct score: {result['correct_score']:.4f} ({correct_variant}) | Top-1 score: {result['top1_score']:.4f} ({result['top1_variant']})")
            print("  Top-5 rankings:")
            for i, (rf, rs, rv) in enumerate(result["top5_rankings"]):
                marker = " <<< CORRECT" if rf == folder else ""
                print(f"    {i+1}. {rs:.4f}  {rf}  [{rv}]{marker}")
...
def save_results(all_results: List[Dict], summary: Dict) -> None:
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("SIGN LANGUAGE RECOGNITION EVALUATION — Method: M5 (DTW+Swap)\n")
        f.write("=" * 80 + "\n\n")
        ...
        if wrong:
            ...
            for r in wrong:
                ...
                f.write(f"  Predicted folder: {r['top1_folder']} (score {r['top1_score']:.4f}, {r['top1_variant']})\n")
                f.write(f"  Correct rank: {r['correct_rank']} (score {r['correct_score']:.4f}, {r['correct_variant']})\n")
                f.write("  Top-5:\n")
                for i, item in enumerate(r.get("top5_rankings", []) or []):
                    if isinstance(item, (list, tuple)) and len(item) >= 3:
                        rf, rs, rv = item[0], float(item[1]), item[2]
                        marker = " <<< CORRECT" if rf == r["folder"] else ""
                        f.write(f"    {i+1}. {rs:.4f}  {rf}  [{rv}]{marker}\n")
                f.write("\n")
        ...
        if correct:
            ...
            f.write("-" * 80 + "\n\n")
            for r in correct:
                f.write(f"  {r['folder']} | {r['user_video']} | score={r['correct_score']:.4f} [{r['correct_variant']}]\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate recognition using Method 5 (DTW + global user hand-swap).")
    ...
    parser.add_argument("--verbose-dtw", action="store_true", help="Show DTW internal prints (very noisy).")
    ...
    print("\n" + "=" * 80)
    print("PHASE 2: Evaluating recognition — Method: M5 (DTW+Swap)")
    print("=" * 80 + "\n")
