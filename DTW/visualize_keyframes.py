"""
Visualize keyframe extraction and DTW alignment
"""
import cv2
import numpy as np
...
def visualize_keyframes_alignment():
    """Show keyframes and their DTW alignment."""
    ...
    # Show which frames were skipped
    all_frames = set(range(matrix1.shape[0]))
    skipped_frames = sorted(all_frames - set(keyframe_indices))
    print(f"\nSkipped frames (similar to previous keyframe):")
    
    # Group consecutive skipped frames
    if skipped_frames:
        groups = []
        start = skipped_frames[0]
        end = skipped_frames[0]
        
        for i in range(1, len(skipped_frames)):
            if skipped_frames[i] == end + 1:
                end = skipped_frames[i]
            else:
