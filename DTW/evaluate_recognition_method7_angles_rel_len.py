"""
Evaluate Method 7: DTW with L1 cost on angles + relative lengths.
"""
...
CACHE_DIR = BASE_DIR / "cache" / "recognition_method7"
...
def precompute_translators(folders: Dict[str, Dict[str, List[Path]]], thr: float) -> Dict[str, Tuple[np.ndarray, List[int]]]:
...
def run_eval(
    ...
    for f_idx, (folder, info) in enumerate(folder_list, 1):
        ...
        for user_path in user_videos:
            ...
            print("  Method: M7 (angles + relative lengths, L1 DTW)")
            ...
            rankings = compare_user_vs_all(
                user_feat, user_idx, translator_data, window_ratio=window_ratio, swap=swap
            )
...
def save_results(all_results: List[Dict], summary: Dict) -> None:
    ...
    with txt_path.open("w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("SIGN LANGUAGE RECOGNITION EVALUATION — Method: M7 (angles + rel lengths)\n")
        f.write("=" * 80 + "\n\n")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Evaluate Method 7 (angles + relative lengths, L1 DTW).")
