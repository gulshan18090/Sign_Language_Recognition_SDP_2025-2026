...
from inference.similarity import VideoMatcher
...
Examples:
    ...
    parser.add_argument(
        '--save-npy-dir',
        type=str,
        help='Save per-video feature matrices as .npy files into this directory'
    )
...
def main():
    ...
    # Validate arguments
    if not args.reference and not args.save_features and not args.save_npy_dir:
    ...
    # Save per-video .npy matrices if requested
    if args.save_npy_dir:
        output_dir = Path(args.save_npy_dir)
        ...
        used_paths = set()
        for video_path, video_features in candidate_features.items():
            video_path_obj = Path(video_path)

            rel_path = None
            if folder_root:
                try:
                    rel_path = video_path_obj.resolve().relative_to(folder_root)
                except Exception:
                    rel_path = None

            if rel_path:
                matrix_path = output_dir / rel_path
                matrix_path = matrix_path.with_suffix('.npy')
                matrix_path.parent.mkdir(parents=True, exist_ok=True)
            else:
                base_name = video_path_obj.stem
                matrix_path = output_dir / f"{base_name}.npy"
                suffix = 1
                while matrix_path in used_paths or matrix_path.exists():
                    matrix_path = output_dir / f"{base_name}_{suffix}.npy"
                    suffix += 1

            np.save(matrix_path, video_features.feature_matrix)
            used_paths.add(matrix_path)
            print(f"Saved .npy: {matrix_path}")
    ...
    if args.reference:
        ...
        if args.visualize and args.method == 'dtw':
            from inference.similarity import DTWVisualizer
