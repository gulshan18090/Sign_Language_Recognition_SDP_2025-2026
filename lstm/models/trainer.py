...
def trainIters(df, encoder, decoder, encodings,
    ...
    early_stopping_enabled = patience is not None and patience > 0
    ...
    for epoch in range(epochs):
        ...
        history_rows.append({
            ...
            "best_val_loss_so_far": float(min(best_val_loss, val_loss)),
            "patience_counter": int(patience_counter)
        })
        
        # Track best validation regardless of early stopping setting
        if val_loss < best_val_loss - min_delta:
            ...
            print("ℹ️ No improvement, but early stopping is disabled (patience <= 0).")
    ...
    if history_path:
        ...
        with open(history_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(history_rows[0].keys()) if history_rows else [
                "epoch", "train_loss", "val_loss", "learning_rate", "best_val_loss_so_far", "patience_counter"
            ])
            writer.writeheader()
            if history_rows:
